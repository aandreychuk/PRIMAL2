...
from Map_Generator import maze_generator, manual_generator
import GroupLock
    ...
    def work(self, max_episode_length, gamma, sess, coord, saver, maps, instances, num_agents):
        ...
        with sess.as_default(), sess.graph.as_default():
            for instance_id in range(len(instances)):
                ...
                if self.agentID == 1:
                    print(instance_id, instances[instance_id]['map_name'], instances[instance_id]['seed'], args.num_agents)
                    ...
                    self.env._reset(manual_generator(maps[instances[instance_id]['map_name']]),
                             {a['agent_id'] + 1: a['start'] for a in instances[instance_id]['task'] if a['agent_id'] < num_agents},
                             {a['agent_id'] + 1: a['goals'][0] for a in instances[instance_id]['task'] if a['agent_id'] < num_agents},
                             {a['agent_id'] + 1: a['goals'][1:] for a in instances[instance_id]['task'] if a['agent_id'] < num_agents})
                ...
                # RL
                    ...
                    while not self.env.finished:  # Give me something!
                        ...
                        if (not agent_done) and (len(episode_buffer) > 1) and (
                    ...
                    if self.agentID == 1:
                        ...
                        self.results.append({'map_name': instances[instance_id]['map_name'],
                                             'seed': instances[instance_id]['seed'],
                                             'num_agents': NUM_THREADS,
                                             'reached_goals': swarm_targets[self.metaAgentID]})
                        with open(f'results/results_{args.map_name}_{args.seed}_{args.num_agents}.json', 'w') as out:
...
parser.add_argument('--map_name', default='mazes-s49_wc2_od50')
parser.add_argument('--seed', default=0)
args = parser.parse_args()
print(args.num_agents, args.map_name)
...
NUM_META_AGENTS = 1
NUM_THREADS = int(args.num_agents)  # int(multiprocessing.cpu_count() / (2 * NUM_META_AGENTS))
...
env_params = [[[WALL_COMPONENTS[0], WALL_COMPONENTS[1]], [OBSTACLE_DENSITY[0], OBSTACLE_DENSITY[1]]] for _ in
              range(NUM_META_AGENTS)]
...
with tf.device("/gpu:0"):
    ...
    for ma in range(NUM_META_AGENTS):
        ...
        gameEnv = PRIMAL2Env(num_agents=num_agents,
                             ...
                             map_generator=manual_generator(maps[instances[0]['map_name']]),
                             IsDiagonal=DIAG_MVMT,
                             isOneShot=IS_ONESHOT,
                             start_poses={a['agent_id'] + 1: a['start'] for a in instances[0]['task'] if a['agent_id'] < num_agents},
                             goal_poses={a['agent_id'] + 1: a['goals'][0] for a in instances[0]['task'] if a['agent_id'] < num_agents},
                             all_goals={a['agent_id'] + 1: a['goals'][1:] for a in instances[0]['task'] if a['agent_id'] < num_agents})
